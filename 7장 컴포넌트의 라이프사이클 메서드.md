...
- 라이프사이클 메서드의 종류는 총 9가지
  - Will 접두사가 붙은 메서드는 어떤 작업을 작동하기 __전__에 실행되는 메서드
  - Did 접두사가 붙은 메서드는 어떤 작업을 작동한 __후__에 실행되는 메서드
...
- 호출되는 메서드는 다음과 같다.
  ...
  - shouldComponentUpdate: 컴포넌트가 리렌더링을 해야 할지 말아야 할지를 결저하는 메서드, true를 반환하면 다음 라이프사이클 계속 실행 
